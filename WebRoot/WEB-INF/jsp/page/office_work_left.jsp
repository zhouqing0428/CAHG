          	...
          	<c:forEach items="${officeWorkCategoryList }" var="ele">
          		<c:if test="${url eq ele.office_work_category_id }">
          			<li style="padding-right:10px;padding-left:10px;margin-top:15px;">
	                	<a style="color: #fff;background: #2b67ac; color: #fff !important" href="dept/officeWorkFileListPage.do?office_work_category_id=${ele.office_work_category_id}"  target="_parent" title='${ele.name }' ><span style="margin-left:20px;">${ele.name }</span></a>
	             	</li>
          		</c:if>
	          	
	          	<c:if test="${url ne ele.office_work_category_id }">
          			<li style="margin-top:15px;padding-right:10px;padding-left:10px;">
	                	<a style="color: #333;" href="dept/officeWorkFileListPage.do?office_work_category_id=${ele.office_work_category_id}"  target="_parent" title='${ele.name }' ><span style="margin-left:20px;">${ele.name }</span></a>
	             	</li>
          		</c:if>
	          	</c:forEach>
          </ul>
