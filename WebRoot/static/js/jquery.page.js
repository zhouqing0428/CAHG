...
(function($){
	...
	function Page(container,fn,args,style){
		...
		if(this.container.constructor!=jQuery){
			alert('page插件第一个参数错误，请录入jQuery对象');
		}
		this.fn=fn||function(){};
		if(typeof(this.fn)!="function"){
			alert('page插件第二个参数错误，请录入函数类型对象');
		}
		//存 args里的属性
		...
		var style=this.style=$.extend({
			...
			"prevNextWidth":70,//上页下页的宽度
			"pagecountWidth":70,//共计多少页的宽度
			"trunWidth":160//跳转模块宽度
		},style||{});
	...
	Page.prototype.setStyle=function(){
		...
		this.obj.find('a.turndown').css({'margin-left':'5px','display':'inline-block','width':'40px','height':s.height-2+'px','line-height':s.height-2+'px'});
